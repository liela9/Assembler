...
void insert_order(char *label, int IC, unsigned long op_code, char *source_op, char *dest_op){
    int source, dest;
    char **struct_access1, **struct_access2;
    unsigned int *orders_table;
    
    orders_table = (unsigned int*)malloc(5 * sizeof(unsigned int));
    ...
    /*
        ...
        Struct: S.1
        register: r1, r2
        Value: 1 (for S.1), 2 (for S.2)  OR  #-5
    */
    if(source = is_register(source_op) != -1){
        if(dest = is_register(dest_op) != -1){/*Two registers*/
            create_order_line(label, IC, op_code, convertDtoB(3), convertDtoB(3), &orders_table);
            create_registers_line(source, dest, IC++, &orders_table);
        }
        else if(label_exists(dest_op)){/*Register and label*/
                create_order_line(label, IC, op_code, convertDtoB(3), convertDtoB(1), &orders_table);
                create_registers_line(source, 0, IC++, &orders_table);
                create_unknown_line(IC++, &orders_table);
            }
            else if(struct_access1 = is_struct(dest_op)){/*Register and struct*/
                    create_order_line(label, IC, op_code, convertDtoB(3), convertDtoB(2), &orders_table);
                    create_registers_line(source, 0, IC++, &orders_table);
                    create_unknown_line(IC++, &orders_table);
                    create_index_line(struct_access1[1], IC++, &orders_table);
                }
    }
        
    else if(label_exists(source_op)){
        if(dest = is_register(dest_op) != -1){/*Label and register*/
            create_order_line(label, IC, op_code, convertDtoB(1), convertDtoB(3), &orders_table);
            create_unknown_line(IC++, &orders_table);
            create_registers_line(0, dest, IC++, &orders_table);
        }
        else if(struct_access1 = is_struct(dest_op)){/*Label and struct*/
                create_order_line(label, IC, op_code, convertDtoB(1), convertDtoB(2), &orders_table);
                create_unknown_line(IC++, &orders_table);
                create_unknown_line(IC++, &orders_table);
                create_index_line(struct_access1[1], IC++, &orders_table);
            }
            else if(label_exists(dest_op)){/*Label and label*/
                    create_order_line(label, IC, op_code, convertDtoB(1), convertDtoB(1), &orders_table);
                    create_unknown_line(IC++, &orders_table);
                    create_unknown_line(IC++, &orders_table);
                }
    }
        ...
        if(dest = is_register(dest_op) != -1){/*Value and register*/
            create_order_line(label, IC, op_code, convertDtoB(0), convertDtoB(3), &orders_table);
            create_value_line(source_op, IC++, &orders_table);
            create_registers_line(0, dest, IC++, &orders_table);
        }
        else if(label_exists(dest_op)){/*Value and label*/
                create_order_line(label, IC, op_code, convertDtoB(0), convertDtoB(1), &orders_table);
                create_value_line(source_op, IC++, &orders_table);
                create_unknown_line(IC++, &orders_table);
            }
            else if(struct_access1 = is_struct(dest_op)){/*Value and struct*/
                    create_order_line(label, IC, op_code, convertDtoB(0), convertDtoB(2), &orders_table);
                    create_value_line(source_op, IC++, &orders_table);
                    create_unknown_line(IC++, &orders_table);
                    create_index_line(struct_access1[1], IC++, &orders_table);
                }
        ...
        if(dest = is_register(dest_op) != -1){/*Struct and register*/
            create_order_line(label, IC, op_code, convertDtoB(2), convertDtoB(3), &orders_table);
            create_unknown_line(IC++, &orders_table);
            create_index_line(struct_access1[1], IC++, &orders_table);
            create_registers_line(0, dest, IC++, &orders_table);
        }
        else if(label_exists(dest_op)){/*Struct and label*/
                create_order_line(label, IC, op_code, convertDtoB(2), convertDtoB(1), &orders_table);
                create_unknown_line(IC++, &orders_table);
                create_index_line(struct_access1[1], IC++, &orders_table);
                create_unknown_line(IC++, &orders_table);
            }
            else if(struct_access2 = is_struct(dest_op)){/*Struct and struct*/
                    create_order_line(label, IC, op_code, convertDtoB(2), convertDtoB(2), &orders_table);
                    create_unknown_line(IC++, &orders_table);
                    create_index_line(struct_access1[1], IC++, &orders_table);
                    create_unknown_line(IC++, &orders_table);
                    create_value_line(struct_access2[1], IC++, &orders_table);
                }
    }
